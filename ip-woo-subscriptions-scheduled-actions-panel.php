...
class IP_WooSubs_Scheduled_Actions_Panel {
	...
	public function render_renewal_status_column( $column, $id ) {
		...
		if ( $this->has_pending_action( 'woocommerce_scheduled_subscription_payment', $subscription->get_id() ) ) {
	 ...
	 * @return bool
	 */
	private function renewal_payment_is_schedulable( $subscription ) {
		$terminal_statuses = array( 'cancelled', 'expired', 'switched', 'trash' );
		if ( in_array( $subscription->get_status(), $terminal_statuses, true ) ) {
			return false;
		}
	 ...
	 * @return bool
	 */
	private function has_pending_action( $hook, $subscription_id ) {
